...
#define MTF02_DMA_RX_BUF_SZ     (256u)
#endif

#ifndef MTF02_RING_SZ
#define MTF02_RING_SZ           (1024u)
#endif

#ifndef MTF02_TICK_MAX_BUDGET
#define MTF02_TICK_MAX_BUDGET   (128u)   /* max. Bytes pro 1 kHz Tick */
#endif
...
#if defined ( __GNUC__ )
__attribute__((aligned(32))) __attribute__((section(".RAM_D2")))
#endif
...
void MTF02_RX_Init(UART_HandleTypeDef *huart8)
{
    ...
    /* ---------------- Vorbereitungen: UART in definierten Zustand bringen ---------------- */
    /* alle RX-/Error-Interrupts der UART erst mal aus */
    __HAL_UART_DISABLE_IT(s_huart, UART_IT_RXNE);
    __HAL_UART_DISABLE_IT(s_huart, UART_IT_PE);
    __HAL_UART_DISABLE_IT(s_huart, UART_IT_ERR);   /* FE/NE/ORE */

    /* evtl. laufende Transfers hart stoppen (idempotent, ok wenn nix läuft) */
    HAL_UART_DMAStop(s_huart);
    HAL_UART_AbortReceive(s_huart);

    /* Error-Flags löschen (insb. ORE = 0x08) + IDLE-Flag löschen */
    ...
    /* Optional robust: Overrun-Erkennung während DMA deaktivieren (empfohlen beim Start) */
    s_huart->Instance->CR3 |= USART_CR3_OVRDIS;

    /* ErrorCode zurücksetzen */
    s_huart->ErrorCode = HAL_UART_ERROR_NONE;

    /* ---------------- DMA+IDLE Empfang starten ---------------- */
    HAL_StatusTypeDef st =
        HAL_UARTEx_ReceiveToIdle_DMA(s_huart, s_dma_rx, MTF02_DMA_RX_BUF_SZ);

    if (st != HAL_OK) {
        /* Diagnosepunkte, falls Start fehlschlägt */
        volatile void *dm = s_huart->hdmarx;        /* darf NICHT NULL sein (LINKDMA?) */
        volatile uint32_t g = s_huart->gState;      /* sollte READY sein */
        volatile uint32_t r = s_huart->RxState;     /* sollte READY sein */
        volatile uint32_t err = s_huart->ErrorCode; /* sollte 0 sein */
        __NOP();                                    /* hier Breakpoint setzen */
        return;                                     /* vorerst raus, um HardFault zu vermeiden */
    }

    /* Half-Transfer-IRQ deaktivieren: wir arbeiten ausschließlich mit IDLE-Events */
    if (s_huart->hdmarx) {
        __HAL_DMA_DISABLE_IT(s_huart->hdmarx, DMA_IT_HT);
    }
    init_of = 1; // init succsessfuly
}
...
//			float vy   = (float)m.flow_vel_y * h_m * 0.01f;
			...
			optical_flow_meas.deltaT = m.time_ms - last_time;
