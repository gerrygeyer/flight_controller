...
//	 return (int16_t)(((uint32_t)__HAL_TIM_GET_COUNTER(&htim4) * (uint32_t)(1<<16)) >> 12);
	return -(int16_t)((uint32_t)__HAL_TIM_GET_COUNTER(&htim4) << 4);
}
...
int16_t read_encoder_rotational_speed(const int16_t frequency){
	...
	rot_speed = CLAMP_INT32_TO_INT16((((int32_t)encoder_count_value - (int32_t)old_encoder_value) * frequency * 360)/max_degree);
