...
VL53L1X_ERROR VL53L1X_SensorInit(uint16_t dev)
{
    ...
    // Lade Default-Konfiguration (93 Register von 0x2D bis 0x87)
    for (Addr = 0x2D; Addr <= 0x87; Addr++) {
        status |= VL53L1_WrByte(dev, Addr, VL51L1X_DEFAULT_CONFIGURATION[Addr - 0x2D]);
    }

    // Starte temporär Messung, um interne Kalibrierung zu triggern
    status |= VL53L1X_StartRanging(dev);

    // Warte bis erstes Datenpaket fertig ist (CheckForDataReady)
    uint8_t tmp = 0;
    while (tmp == 0) {
        status |= VL53L1X_CheckForDataReady(dev, &tmp);
    }

    // Unterbrechung löschen und Messung stoppen
    status |= VL53L1X_ClearInterrupt(dev);
    status |= VL53L1X_StopRanging(dev);

    // Zwei wichtige Default-Werte setzen (empfohlen von ST)
    status |= VL53L1_WrByte(dev, VL53L1_VHV_CONFIG__TIMEOUT_MACROP_LOOP_BOUND, 0x09); // two bounds VHV
    status |= VL53L1_WrByte(dev, 0x0B, 0);                                             // start VHV from previous temp

    // DistanceMode explizit setzen (Long Mode als Default)
